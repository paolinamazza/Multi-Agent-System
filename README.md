...
- **hardcoded_DataProcessAgent_Tools.py** and **local_agent_project.py**: Two files containing the code that was not included in our official version but mentioned in the README.
...
To measure the performance of the system, we calculated the **Accuracy** (% of correct answers). In our current evaluation, the system achieved **96% accuracy**, successfully answering 22 out of 23 queries. These included both numerical insights and visualizations across various levels of complexity.

Of course, while building our agents we tested them numerous times: as most agent systems, it can hallucinate sometimes. For example, you can run the same question several times: it will answer correctly almost always, but it can return an error occasionally. This depends, of course, on the complexity and vagueness of the user request.

During the project, we always compared the agents generated Python code to our manually written code to improve the agents. The biggest difficulty we encountered was generating code for queries that required merging datasets; an issue we solved by specifying the necessary steps in the InsightBuilder tool. Although the agents will respond almost always correctly, it remains the type of question that can create the most problems.
